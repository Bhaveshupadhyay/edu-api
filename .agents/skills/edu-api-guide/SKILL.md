...
- **`users`**: `id`, `email`, `password`, `stripe_customer_id`.
- **`user_devices`**: `id`, `user_id`, `device_id`, `device_type` (`'android'`, `'ios'`, `'web'`, `'tv'`), `rem_token`.
- **`user_profiles`**: `id`, `user_id`, `device_id`, `name`, `bio`.
- **`plans`**: `id`, `plan_name`, `monthly_price`, `duration_value`, `duration_unit`, `max_screens`, `stripe_price_id`, `is_active`.
- **`user_subscriptions`**: `id`, `user_id`, `plan_id`, `stripe_sub_id`, `status` (`'active'`, `'trialing'`, `'past_due'`, `'canceled'`), `current_period_end`, `cancel_at_period_end`.
- **Content Hierarchy**:
  - `home_page_config` ➔ `nav_pills` ➔ `nav_pill_collections` ➔ `collections` ➔ `collection_modules` ➔ `modules` ➔ `syllabus` ➔ `lessons` ➔ `videos`.
...
const data = await getOrSetCache("cache:/api/v1/users/home*", async () => {
  return await db.query(...);
}, 3600);

// Invalidate matching keys
await clearCache("user_profile:123*");
```
...
- **`ACCESS_TOKEN_SECRET`**: Standard Bearer token authentication ([middleware/auth.middleware.js](file:///home/runner/work/github_automation/github_automation/workspace_target/middleware/auth.middleware.js)).
- **`REFRESH_TOKEN_SECRET`**: Long-lived refresh token stored in HTTP-only cookie (`XXAFIT`) and DB `user_devices.rem_token`.
- **`SHORT_TOKEN_SECRET`**: Password reset authorization ([middleware/reset.middleware.js](file:///home/runner/work/github_automation/github_automation/workspace_target/middleware/reset.middleware.js)).
...
Emails matching `process.env.REVIEWER_EMAIL` (`isReviewer(email)` in [utils/authHelper.js](file:///home/runner/work/github_automation/github_automation/workspace_target/utils/authHelper.js)) bypass active subscription checks to enable App Store / Google Play review approval.

---

## 6. Payments & Realtime Notification Architecture

### Checkout Security Pattern
To prevent parameter tampering during subscription checkout:
1. `get_checkout_options` computes `HMAC-SHA256` checksum of `${user_id}:${plan}:${device_id}:${device}` with `CHECKSUM_SECRET`.
2. `post_subscription` re-computes checksum and verifies using `crypto.timingSafeEqual` before creating Stripe checkout sessions.
...
### Socket.IO Cross-Process Pub/Sub
...
- If `io` instance exists (main server process): Emits directly to Socket.IO room (`user_${userId}` or `device_${deviceId}`).
- If `io` does NOT exist (standalone worker process): Publishes to Redis channel `socket-notifications`. Main server receives message and forwards to Socket.IO clients.
...
### Standard Scripts ([package.json](file:///home/runner/work/github_automation/github_automation/workspace_target/package.json))
- **Start Web API**: `npm start` (`node index`)
- **Start Web API (Dev / Hot Reload)**: `npm run dev` (`nodemon index`)
- **Start Worker**: `npm run worker` (`node workers/webhookWorker.js`)
- **Start Worker (Dev)**: `npm run dev:worker` (`nodemon workers/webhookWorker.js`)
- **Generate Swagger Spec**: `npm run swagger` (`node swagger.js`)
- **Clear Redis Cache**: `npm run clear-cache` (`node utils/clearCacheScript.js`)

### Code Quality & Syntax Verification
- **ESLint Check**: `npx eslint .`
- **Node Syntax Check**: `node --check index.js`

### Production Deployment
- Deploys to **Railway** via GitHub Actions ([.github/workflows/deploy.yml](file:///home/runner/work/github_automation/github_automation/workspace_target/.github/workflows/deploy.yml)).
- Service 1: `edu-api` (Web API).
- Service 2: `distinguished-tranquility` (Background Webhook Worker).
